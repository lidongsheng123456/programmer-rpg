...
struct GuardianListView: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    GlassCard {
                        HStack {
                            VStack(alignment: .leading, spacing: AppTheme.spacingSM) {
                                Text("Guardian Overview").font(AppTypography.heading3).foregroundStyle(AppColors.textPrimary)
                                let online = viewModel.serverStatuses.filter(\.isOnline).count
                                Text("\(online)/\(viewModel.serverStatuses.count) Online").font(AppTypography.bodyMedium)
                            ...
                            GlassCard {
                                HStack(spacing: AppTheme.spacingMD) {
                                    ...
                                    VStack(alignment: .trailing, spacing: AppTheme.spacingXS) {
                                        StatusBadge(status.isOnline ? .online : .offline, label: status.isOnline ? "Online" : "Offline")
            ...
            .navigationTitle("Server Guardians").navigationBarTitleDisplayMode(.large)
