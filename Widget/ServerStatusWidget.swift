...
struct ServerStatusProvider: TimelineProvider {
    func placeholder(in context: Context) -> ServerStatusEntry { ServerStatusEntry(date: .now, servers: [.init(name: "Main Server", emoji: "\u{1F43B}", isOnline: true, responseTimeMs: 42)]) }
...
struct ServerStatusWidgetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack { Image(systemName: "server.rack").font(.system(size: 12)).foregroundStyle(Color(hex: 0x7C3AED)); Text("Guardian Status").font(.system(size: 12, weight: .semibold)) }
            ForEach(Array(entry.servers.prefix(3).enumerated()), id: \.offset) { _, s in
                HStack(spacing: 8) { Text(s.emoji).font(.system(size: 16)); Text(s.name).font(.system(size: 11)).lineLimit(1); Spacer()
                    ...
                    Text(s.isOnline ? "\(s.responseTimeMs)ms" : "Offline").font(.system(size: 9, design: .monospaced)).foregroundStyle(.secondary) }
            }
...
struct ServerStatusWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: ServerStatusProvider()) { ServerStatusWidgetView(entry: $0) }
            .configurationDisplayName("Server Status").description("Monitor guardian online status").supportedFamilies([.systemSmall])
    }
}
