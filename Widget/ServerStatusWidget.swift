...
struct ServerStatusProvider: TimelineProvider {
    func placeholder(in context: Context) -> ServerStatusEntry { ServerStatusEntry(date: .now, servers: [.init(name: "涓绘湇鍔″櫒", emoji: "馃悏", isOnline: true, responseTimeMs: 42)]) }
...
struct ServerStatusWidgetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack { Image(systemName: "server.rack").font(.system(size: 12)).foregroundStyle(Color(hex: 0x7C3AED)); Text("瀹堟姢鍏界姸鎬?).font(.system(size: 12, weight: .semibold)) }
            ForEach(Array(entry.servers.prefix(3).enumerated()), id: \.offset) { _, s in
                HStack(spacing: 8) { Text(s.emoji).font(.system(size: 16)); Text(s.name).font(.system(size: 11)).lineLimit(1); Spacer()
                    ...
                    Text(s.isOnline ? "\(s.responseTimeMs)ms" : "绂荤嚎").font(.system(size: 9, design: .monospaced)).foregroundStyle(.secondary) }
            }
...
struct ServerStatusWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: ServerStatusProvider()) { ServerStatusWidgetView(entry: $0) }
            .configurationDisplayName("鏈嶅姟鍣ㄧ姸鎬?).description("鐩戞帶瀹堟姢鍏藉湪绾跨姸鎬?).supportedFamilies([.systemSmall])
    }
}