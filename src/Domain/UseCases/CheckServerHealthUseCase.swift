...
final class CheckServerHealthUseCase: @unchecked Sendable {
    ...
    init(serverRepo: ServerRepositoryProtocol) {
    ...
    func execute() async throws -> [ServerStatus] {
        ...
        for status in statuses {
            var history = serverRepo.getHistory(for: status.config.id)
                ?? ServerPingHistory(configId: status.config.id, records: [])
            history.records.append(
                ServerPingHistory.PingRecord(
            ...
            history.records = history.records.filter { $0.timestamp > cutoff }
